...
- Desktop host app: anything the console needs from the daemon must come from an existing sidecar subcommand (`status --json`, `pair`, …). If the CLI lacks it, add it to the alleycat fork first; do not scrape log files or reimplement daemon logic in the app.

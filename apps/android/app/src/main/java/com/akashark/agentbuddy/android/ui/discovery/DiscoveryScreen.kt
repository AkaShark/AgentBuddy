...
private fun ServerRow(
    ...
    Row(
        ...
        Icon(
            ...
            tint = if (entry.hasCodexServer) AgentBuddyTheme.accent else AgentBuddyTheme.textMuted,
            modifier = Modifier.size(20.dp),
        )
        Spacer(Modifier.width(10.dp))
        Column(modifier = Modifier.weight(1f)) {
            Text(entry.name.ifBlank { entry.hostname }, color = AgentBuddyTheme.textPrimary, fontSize = 14.sp)
            Text(subtitle, color = AgentBuddyTheme.textSecondary, fontSize = 11.sp)
        }
        val (sourceColor, sourceLabel) = when (entry.source) {
            "bonjour" -> AgentBuddyTheme.info to "Bonjour"
            "tailscale" -> Color(0xFFC797D8) to "Tailscale"
            "lanProbe" -> AgentBuddyTheme.accent to "LAN"
            "arpScan" -> AgentBuddyTheme.textSecondary to "ARP"
            "ssh" -> Color(0xFFFF9500) to "SSH"
            "local" -> AgentBuddyTheme.accent to "本地"
            else -> AgentBuddyTheme.textMuted to "手动"
        }
        Text(
            text = sourceLabel,
            color = sourceColor,

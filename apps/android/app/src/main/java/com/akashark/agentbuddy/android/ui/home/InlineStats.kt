    ...
    Row(
            ...
            IconCountChip(
                icon = { size ->
                    Icon(
                        ...
                        tint = AgentBuddyTheme.textMuted.copy(alpha = 0.7f),
        ...
        if (additions > 0 || deletions > 0) {
            Row(
                ...
                Text(
                    text = "-$deletions",
                    color = AgentBuddyTheme.danger.copy(alpha = 0.6f),
                    fontFamily = AgentBuddyTheme.monoFont,
                    fontSize = CHIP_FONT_SP.scaled,
                )
            }
        ...
        if (tokenUsage != null && window != null && window > 0L) {
            ...
            val color = if (pct > 80) {
                AgentBuddyTheme.warning.copy(alpha = 0.8f)
            } else {
                AgentBuddyTheme.textMuted.copy(alpha = 0.7f)
            }
            Text(
                text = "$pct%",
                color = color,

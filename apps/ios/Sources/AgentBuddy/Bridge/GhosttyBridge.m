...
static NSString *const AgentBuddyGhosttyErrorDomain = @"com.akashark.agentbuddy.ghostty";

static void AgentBuddyGhosttyResizeBackingLayers(UIView *view, CGFloat scale);
static void AgentBuddyGhosttyWakeup(void *userdata);
static bool AgentBuddyGhosttyAction(ghostty_app_t app, ghostty_target_s target, ghostty_action_s action);
static bool AgentBuddyGhosttyReadClipboard(void *userdata, ghostty_clipboard_e clipboard, void *request);
static void AgentBuddyGhosttyConfirmReadClipboard(void *userdata, const char *title, void *request, ghostty_clipboard_request_e requestType);
static void AgentBuddyGhosttyWriteClipboard(void *userdata, ghostty_clipboard_e clipboard, const ghostty_clipboard_content_s *contents, size_t count, bool confirm);
static void AgentBuddyGhosttyCloseSurface(void *userdata, bool processActive);
static void AgentBuddyGhosttyExternalWrite(void *userdata, const uint8_t *data, uintptr_t length);
static dispatch_queue_t AgentBuddyGhosttyDestroyQueue(void);

@interface AgentBuddyGhosttyTerminal ()
@property (nonatomic, weak) UIView *view;
- (void)draw;
@end

@implementation AgentBuddyGhosttyTerminal {
...
+ (BOOL)ensureInitialized:(NSError **)error {
    ...
    if (error != NULL) {
        *error = [NSError errorWithDomain:AgentBuddyGhosttyErrorDomain
                                     code:initResult
...
- (nullable instancetype)initWithView:(UIView *)view error:(NSError **)error {
    ...
    if (![AgentBuddyGhosttyTerminal ensureInitialized:error]) {
        return nil;
    }
    ...
    runtimeConfig.wakeup_cb = AgentBuddyGhosttyWakeup;
    runtimeConfig.action_cb = AgentBuddyGhosttyAction;
    runtimeConfig.read_clipboard_cb = AgentBuddyGhosttyReadClipboard;
    runtimeConfig.confirm_read_clipboard_cb = AgentBuddyGhosttyConfirmReadClipboard;
    runtimeConfig.write_clipboard_cb = AgentBuddyGhosttyWriteClipboard;
    runtimeConfig.close_surface_cb = AgentBuddyGhosttyCloseSurface;
    ...
    surfaceConfig.external_pty_write = AgentBuddyGhosttyExternalWrite;
    ...
    AgentBuddyGhosttyResizeBackingLayers(view, surfaceConfig.scale_factor);
...
- (void)invalidate {
    ...
    if (surface != NULL || app != NULL || config != NULL) {
        AgentBuddyGhosttyTerminal *terminal = self;
        dispatch_async(AgentBuddyGhosttyDestroyQueue(), ^{
            (void)terminal;
            if (surface != NULL) {
...
- (void)resizeToWidth:(CGFloat)width height:(CGFloat)height scale:(CGFloat)scale {
    ...
    AgentBuddyGhosttyResizeBackingLayers(_view, resolvedScale);
...
- (AgentBuddyGhosttySurfaceMetrics)surfaceMetrics {
    AgentBuddyGhosttySurfaceMetrics out = {0};
    if (_surface == NULL) {
        return out;
    }
...
static ghostty_input_key_e AgentBuddyGhosttyKeyToGhosttyKey(AgentBuddyGhosttyKey key) {
    switch (key) {
        case AgentBuddyGhosttyKeyEnter:      return GHOSTTY_KEY_ENTER;
        case AgentBuddyGhosttyKeyTab:        return GHOSTTY_KEY_TAB;
        case AgentBuddyGhosttyKeyBackspace:  return GHOSTTY_KEY_BACKSPACE;
        case AgentBuddyGhosttyKeyEscape:     return GHOSTTY_KEY_ESCAPE;
        case AgentBuddyGhosttyKeySpace:      return GHOSTTY_KEY_SPACE;
        case AgentBuddyGhosttyKeyArrowUp:    return GHOSTTY_KEY_ARROW_UP;
        case AgentBuddyGhosttyKeyArrowDown:  return GHOSTTY_KEY_ARROW_DOWN;
        case AgentBuddyGhosttyKeyArrowLeft:  return GHOSTTY_KEY_ARROW_LEFT;
        case AgentBuddyGhosttyKeyArrowRight: return GHOSTTY_KEY_ARROW_RIGHT;
        case AgentBuddyGhosttyKeyPageUp:     return GHOSTTY_KEY_PAGE_UP;
        case AgentBuddyGhosttyKeyPageDown:   return GHOSTTY_KEY_PAGE_DOWN;
        case AgentBuddyGhosttyKeyHome:       return GHOSTTY_KEY_HOME;
        case AgentBuddyGhosttyKeyEnd:        return GHOSTTY_KEY_END;
        case AgentBuddyGhosttyKeyDelete:     return GHOSTTY_KEY_DELETE;
        case AgentBuddyGhosttyKeyInsert:     return GHOSTTY_KEY_INSERT;
        case AgentBuddyGhosttyKeyUnidentified:
        default:                         return GHOSTTY_KEY_UNIDENTIFIED;
    }
}

- (BOOL)dispatchKeyAction:(int)action
                      key:(AgentBuddyGhosttyKey)key
                     mods:(int)mods
                     text:(NSString *)text
    ...
    event.keycode = (uint32_t)AgentBuddyGhosttyKeyToGhosttyKey(key);
...
static dispatch_queue_t AgentBuddyGhosttyDestroyQueue(void) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        queue = dispatch_queue_create("com.akashark.agentbuddy.ghostty.destroy", attr);
    });
    return queue;
}

static AgentBuddyGhosttyTerminal *AgentBuddyGhosttyTerminalFromUserdata(void *userdata) {
    if (userdata == NULL) {
        return nil;
    }
    return (__bridge AgentBuddyGhosttyTerminal *)userdata;
}

static void AgentBuddyGhosttyWakeup(void *userdata) {
    AgentBuddyGhosttyTerminal *terminal = AgentBuddyGhosttyTerminalFromUserdata(userdata);
    if (terminal == nil) {
        return;
    }

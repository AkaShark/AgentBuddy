...
/// `AgentBuddyGhosttyTerminal` and hops every Ghostty C call onto the main
...
final class GhosttyRendererBackendBridge: TerminalRendererBackend, @unchecked Sendable {
    private weak var terminal: AgentBuddyGhosttyTerminal?
    ...
    init(terminal: AgentBuddyGhosttyTerminal) {
    ...
    func dispatchKey(event: TerminalKeyEvent) {
        ...
        let agentBuddyKey = GhosttyKeyTranslator.agentBuddyKey(for: event.code)
...
/// Translation: Rust `TerminalKey*` → bridge-level `AgentBuddyGhosttyKey`.
...
enum GhosttyKeyTranslator {
    ...
    static func agentBuddyKey(for value: TerminalKeyCode) -> AgentBuddyGhosttyKey {
        switch value {

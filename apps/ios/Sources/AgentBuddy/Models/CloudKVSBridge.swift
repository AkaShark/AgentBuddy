...
final class CloudKVSBridge {
    ...
    private static let envelopeKey = "agentbuddy.cloud_sync_envelope_v1"
    /// Locally-persisted device identifier for `source_device` tagging in
    /// the envelope. Stable across launches but not synced.
    private static let deviceIdKey = "agentbuddy.cloud_sync_device_id"
    ...
    private func applyEnvelopeFromStore() {
        ...
        do {
            ...
            NotificationCenter.default.post(name: .agentBuddyThreadPreferencesDidChange, object: nil)
        } catch {
            LLog.warn(
                "cloud_sync",

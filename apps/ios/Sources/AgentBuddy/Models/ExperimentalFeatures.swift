...
enum AgentBuddyFeature: String, CaseIterable, Identifiable {
    ...
    var description: String {
        ...
        case .appleWatch: return String(localized: "Push server, task, and approval state to a paired Apple Watch. Requires the AgentBuddy watch app to be installed.")
...
final class ExperimentalFeatures {
    ...
    @ObservationIgnored private let key = "agentbuddy.experimentalFeatures"
    ...
    func isEnabled(_ feature: AgentBuddyFeature) -> Bool {
        overrides[feature.rawValue] ?? feature.defaultEnabled
    }

    func setEnabled(_ feature: AgentBuddyFeature, _ value: Bool) {
        var map = overrides

...
final class MacPairingHost: NSObject {
    ...
    private static let macIdUserDefaultsKey = "agentbuddy.mac_pair_id"
    ...
    /// the iPhone at a working local Codex when `AgentBuddyPlatform.isDirectDistMac`
    ...
    func startIfNeeded() {
        guard AgentBuddyPlatform.isCatalyst else { return }
        guard !isBroadcasting else { return }
        isBroadcasting = true
        let macId = Self.resolveMacId()
        let deviceName = AgentBuddyPlatform.localRuntimeDisplayName()
    ...
    private func showConfirmDialog(distanceM: Float?) {
        ...
        let alert = UIAlertController(
            ...
            message: "\(deviceLabel)\(distanceSuffix) wants to pair with this Mac as its AgentBuddy home base.",

...
extension Notification.Name {
    static let themeDidChange = Notification.Name("com.akashark.agentbuddy.themeDidChange")
}

/// Thread-safe store for resolved themes, accessible from any isolation context.
/// ThemeManager writes here; AgentBuddyTheme reads from here.
...
enum AgentBuddyAppearanceMode: String, CaseIterable, Identifiable {
    case system
    case light
    case dark
...
final class ThemeManager {
    ...
    func setAppearanceMode(_ mode: AgentBuddyAppearanceMode) {
    ...
    private static func storedAppearanceMode() -> AgentBuddyAppearanceMode {
        guard let raw = UserDefaults.standard.string(forKey: appearanceModeKey) else {
            return .system
        }
        return AgentBuddyAppearanceMode(rawValue: raw) ?? .system
    }

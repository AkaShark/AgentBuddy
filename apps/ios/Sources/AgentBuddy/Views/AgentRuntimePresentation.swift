...
/// app launch in `AgentBuddyApp` so any view can resolve an `AgentId` to
...
extension AgentRuntimeKind {
    ...
    /// `AgentIconView`. AgentBuddy ships icons for the agents it knows
    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: size * 0.2)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: size * 0.2)
                        .stroke(AgentBuddyTheme.textPrimary.opacity(0.25), lineWidth: 0.5)
                )
            Text(monogramLetter)
        ...
        Text("BETA")
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 3)
                    .stroke(AgentBuddyTheme.accent.opacity(0.6), lineWidth: 0.5)
            )
    }
}

...
struct InlineVoiceStatusStrip: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            HStack(spacing: 5) {
                Circle()
                    .fill(session.isListening ? AgentBuddyTheme.accent : AgentBuddyTheme.textMuted.opacity(0.4))
                    .frame(width: 5, height: 5)
                Text("YOU")
                    .font(AgentBuddyFont.monospaced(.caption2, weight: .bold))
                    .foregroundColor(session.isListening ? AgentBuddyTheme.textPrimary : AgentBuddyTheme.textMuted)
                AudioWaveformView(level: inputLevel, tint: AgentBuddyTheme.accent)
                    .frame(width: 48, height: 14)
            }

            HStack(spacing: 5) {
                Circle()
                    .fill(session.isSpeaking ? AgentBuddyTheme.warning : AgentBuddyTheme.textMuted.opacity(0.4))
                    .frame(width: 5, height: 5)
                Text("CODEX")
                    .font(AgentBuddyFont.monospaced(.caption2, weight: .bold))
                    .foregroundColor(session.isSpeaking ? AgentBuddyTheme.textPrimary : AgentBuddyTheme.textMuted)
                AudioWaveformView(level: outputLevel, tint: AgentBuddyTheme.warning)
                    .frame(width: 48, height: 14)
            }
            ...
            Button(action: onToggleSpeaker) {
                HStack(spacing: 4) {
                    ...
                    Text(session.route.label)
                        .font(AgentBuddyFont.styled(.caption2, weight: .semibold))
                }

...
struct MinigameOverlayView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            header
            Divider().background(AgentBuddyTheme.textSecondary.opacity(0.2))
            content
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .stroke(AgentBuddyTheme.textSecondary.opacity(0.15), lineWidth: 0.5)
        )
    ...
    private func shimmerBar(height: CGFloat) -> some View {
        RoundedRectangle(cornerRadius: 8, style: .continuous)
            .fill(LinearGradient(
                colors: [
                    AgentBuddyTheme.textSecondary.opacity(0.18),
                    AgentBuddyTheme.accent.opacity(0.4),
                    AgentBuddyTheme.textSecondary.opacity(0.18),
                ],
        ...
        VStack(spacing: 12) {
            ...
            Button(action: onRetry) {
                Text("Try again")
                    ...
                    .padding(.vertical, 8)
                    .overlay(
                        Capsule().stroke(AgentBuddyTheme.accent.opacity(0.5), lineWidth: 1)
                    )
            }
            .buttonStyle(.plain)
            Spacer()

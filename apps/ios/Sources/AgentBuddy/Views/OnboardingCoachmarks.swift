...
struct OnboardingCoachmarksView: View {
    ...
    /// is overlaid externally (in `AgentBuddyApp.primaryNavigationStack`) so its
...
private struct CoachmarkHalo: View {
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 30.0)) { context in
            ...
            Circle()
                .stroke(AgentBuddyTheme.accent.opacity(0.7), lineWidth: 1.5)
                .background(
                    Circle()
                        .stroke(AgentBuddyTheme.accent.opacity(outerOpacity), lineWidth: outerWidth)
                        .blur(radius: 2)
                )
                .scaleEffect(scale)
        }
...
private struct CoachmarkArrow: View {
    ...
    var body: some View {
        Canvas { ctx, _ in
            ...
            ctx.stroke(
                path,
                with: .color(AgentBuddyTheme.accent.opacity(isPrimary ? 0.95 : 0.80)),
            ...
            head.closeSubpath()
            ctx.fill(head, with: .color(AgentBuddyTheme.accent.opacity(isPrimary ? 0.95 : 0.9)))
        }
    }

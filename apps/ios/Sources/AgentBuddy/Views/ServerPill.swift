    ...
    var body: some View {
        ...
        .overlay(
            Capsule(style: .continuous)
                .stroke(
                    isSelected ? AgentBuddyTheme.accent.opacity(0.75) : AgentBuddyTheme.textMuted.opacity(0.25),
...
private struct AgentRuntimeOverflowBadge: View {
    ...
    var body: some View {
        Text("+\(count)")
            .agentBuddyMonoFont(size: 9, weight: .bold)
            .foregroundStyle(AgentBuddyTheme.textPrimary)
            .lineLimit(1)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 5, style: .continuous)
                    .stroke(AgentBuddyTheme.textPrimary.opacity(0.28), lineWidth: 0.55)
            )

...
struct UWBDebugView: View {
        ...
        card(title: "Distance") {
            HStack(alignment: .firstTextBaseline, spacing: 6) {
                Text(pairing.lastDistance.map { String(format: "%.2f", $0) } ?? "—")
                    .agentBuddyFont(size: 36, weight: .semibold)
    ...
    private func row(_ key: String, value: String) -> some View {
        HStack {
            ...
            Spacer()
            Text(value)
                .agentBuddyFont(.footnote, weight: .medium)
                .foregroundColor(AgentBuddyTheme.textPrimary)
        }
    }
}
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack(alignment: .leading) {
                Capsule().fill(AgentBuddyTheme.surface)
                    .frame(height: 6)
...
private struct CompassView: View {
    ...
    var body: some View {
        ZStack {
            ...
            Circle()
                .fill(AgentBuddyTheme.surface.opacity(0.4))
            // Crosshair
            Path { p in

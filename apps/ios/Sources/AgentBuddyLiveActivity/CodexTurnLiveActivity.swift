...
struct CodexTurnLiveActivity: Widget {
    ...
    private var warningColor: Color { AgentBuddyPalette.warning.color(for: .dark) }
    private var dangerColor: Color { AgentBuddyPalette.danger.color(for: .dark) }
    private var primaryText: Color { AgentBuddyPalette.textPrimary.color(for: .dark) }
    private var secondaryText: Color { AgentBuddyPalette.textSecondary.color(for: .dark) }
    private var mutedText: Color { AgentBuddyPalette.textMuted.color(for: .dark) }
        ...
        ActivityConfiguration(for: CodexTurnAttributes.self) { context in
            ...
            DynamicIsland {
                DynamicIslandExpandedRegion(.leading) {
                    agentBuddyLogo(size: 18)
                }
                DynamicIslandExpandedRegion(.center) {
                    VStack(alignment: .leading, spacing: 3) {
                        Text(context.attributes.prompt)
                            .font(.system(size: 11, weight: .medium, design: AgentBuddyPalette.fontDesign))
                        ...
                        HStack(spacing: 6) {
                            ...
                            if context.state.toolCallCount > 0 {
                                Label("\(context.state.toolCallCount)", systemImage: "chevron.left.forwardslash.chevron.right")
                                    .font(.system(size: 9, design: AgentBuddyPalette.fontDesign))
    ...
    private func agentBuddyLogo(size: CGFloat) -> some View {
        Image("brand_logo")
            .resizable()
            .interpolation(.high)
            .scaledToFit()
    ...
    private func phaseBadge(_ state: CodexTurnAttributes.ContentState) -> some View {
        Text(phaseBadgeText(state))
            .font(.system(size: 10, weight: .medium, design: AgentBuddyPalette.fontDesign))
    ...
    private func metaChip(systemImage: String, text: String) -> some View {
        HStack(spacing: 4) {
            ...
            Text(text)
                .font(.system(size: 10, design: AgentBuddyPalette.fontDesign))
                .foregroundStyle(mutedText)
                .lineLimit(1)
        }
        .padding(.trailing, 10)
    ...
    private func ctxBadge(_ percent: Int) -> some View {
        Text("\(percent)%")
            .font(.system(size: 9, weight: .semibold, design: AgentBuddyPalette.fontDesign))
    ...
    private func liveTimer(context: ActivityViewContext<CodexTurnAttributes>, size: CGFloat) -> some View {
        if isActive(context.state) {
            Text(timerInterval: context.attributes.startDate...Date.distantFuture, countsDown: false)
                .font(.system(size: size, design: AgentBuddyPalette.fontDesign))
                .monospacedDigit()
...
private struct AdaptiveLiveTimer: View {
    ...
    private var timerColor: Color {
        let text = AgentBuddyPalette.textSecondary.color(for: colorScheme)
    ...
    var body: some View {
        ...
        .font(.system(size: 15, design: AgentBuddyPalette.fontDesign))
        .monospacedDigit()
        .fontWeight(.regular)

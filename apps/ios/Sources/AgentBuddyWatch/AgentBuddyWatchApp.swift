...
/// `AgentBuddyWatchApp` so the singleton survives the whole app lifetime.
...
final class AgentBuddyWatchAppDelegate: NSObject, WKApplicationDelegate {
    static let shared = AgentBuddyWatchAppDelegate()
...
/// Root @main for the AgentBuddy Watch app. Vertically paginated TabView
/// makes the three hero surfaces reachable via crown/swipe.
@main
struct AgentBuddyWatchApp: App {
    @WKApplicationDelegateAdaptor(AgentBuddyWatchAppDelegate.self) private var appDelegate
    ...
    var body: some Scene {
        ...
        WKNotificationScene(
            controller: AgentBuddyNotificationController.self,
...
final class AgentBuddyNotificationController: WKUserNotificationHostingController<NotificationScreen> {

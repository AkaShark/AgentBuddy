...
struct OpenServerOnWatchIntent: AppIntent {
    ...
    static let description = IntentDescription(
        "Open the AgentBuddy watch app focused on a specific Codex server."
    )
    ...
    func perform() async throws -> some IntentResult {
        let url = URL(string: "agentbuddy-watch://server/\(server.id)")!
        await MainActor.run {
...
struct StartVoiceOnWatchIntent: AppIntent {
    ...
    static let description = IntentDescription(
        "Open AgentBuddy and switch to the realtime voice screen."
    )
    static let openAppWhenRun: Bool = true

...
/// Pure deep-link parser for `agentbuddy-watch://…` URLs. The watch root view
...
final class WatchDeepLinkRouter: ObservableObject {
    ...
    /// Called by `AgentBuddyWatchApp.onOpenURL` AND by AppIntent `.perform`
    ...
    nonisolated static func destination(for url: URL) -> Destination? {
        guard url.scheme == "agentbuddy-watch" else { return nil }
        switch url.host {
        case "task":

...
final class WatchSessionBridge: NSObject, WCSessionDelegate {
    ...
    /// `.agentBuddyThreadPreferencesDidChange` and triggers a refreshed push.
    ...
    private func reloadComplicationsIfChanged(store: WatchAppStore) {
        ...
        center.reloadTimelines(ofKind: "AgentBuddyCircularComplication")
        center.reloadTimelines(ofKind: "AgentBuddyCornerComplication")
        center.reloadTimelines(ofKind: "AgentBuddyRectangularComplication")
    }

    private nonisolated func handle(_ payload: [String: Any]) {
        guard
            let raw = payload["agentbuddy.snapshot"] as? Data,

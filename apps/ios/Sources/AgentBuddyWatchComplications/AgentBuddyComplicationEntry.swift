...
/// through the `group.com.akashark.agentbuddy` App Group — the iOS app writes
/// the current running-task snapshot into `UserDefaults` and complications
/// read it on each reload.
struct AgentBuddyComplicationEntry: TimelineEntry {
    ...
    static let placeholder = AgentBuddyComplicationEntry(
        date: .now,
        mode: .running,
    ...
    static let idlePlaceholder = AgentBuddyComplicationEntry(
        date: .now,
        mode: .idle,
...
struct AgentBuddyComplicationPayload: Codable, Equatable {
...
enum AgentBuddyComplicationStore {
    static let appGroup = "group.com.akashark.agentbuddy"
    private static let key = "complication.snapshot.v1"

    static func current() -> AgentBuddyComplicationEntry {
        guard let payload = currentPayload() else { return .placeholder }
        return entry(from: payload)
    }

    static func currentPayload() -> AgentBuddyComplicationPayload? {
        guard
    ...
    static func entry(from payload: AgentBuddyComplicationPayload, at date: Date = .now) -> AgentBuddyComplicationEntry {
    ...
    static func write(_ entry: AgentBuddyComplicationEntry) {

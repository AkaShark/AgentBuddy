...
struct AgentBuddyRunningTurnWidget: Widget {
    let kind = "AgentBuddyRunningTurnWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: RunningTurnTimelineProvider()) { entry in
            AgentBuddyRunningTurnView(entry: entry)
                .widgetAccentable()
...
struct AgentBuddyRunningTurnEntry: TimelineEntry {
    let date: Date
    /// `nil` snapshot tells the view to render a hidden/empty placeholder.
    let snapshot: RunningTurnSnapshot?

    static let placeholder = AgentBuddyRunningTurnEntry(
        date: .now,
    ...
    static let empty = AgentBuddyRunningTurnEntry(date: .now, snapshot: nil)
}
...
struct RunningTurnTimelineProvider: TimelineProvider {
    typealias Entry = AgentBuddyRunningTurnEntry

    func placeholder(in context: Context) -> AgentBuddyRunningTurnEntry {
        .placeholder
    }

    func getSnapshot(in context: Context, completion: @escaping (AgentBuddyRunningTurnEntry) -> Void) {
        completion(currentEntry())
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<AgentBuddyRunningTurnEntry>) -> Void) {
        let now = Date()
        ...
        guard let snapshot = entry.snapshot, !RunningTurnStore.isStale(snapshot, now: now) else {
            ...
            let timeline = Timeline(
                entries: [AgentBuddyRunningTurnEntry(date: now, snapshot: nil)],
        ...
        var entries: [AgentBuddyRunningTurnEntry] = []
        for step in 0..<30 {
            entries.append(AgentBuddyRunningTurnEntry(
    ...
    private func currentEntry(at now: Date = .now) -> AgentBuddyRunningTurnEntry {
        guard let snapshot = RunningTurnStore.current(),
              !RunningTurnStore.isStale(snapshot, now: now) else {
            return AgentBuddyRunningTurnEntry(date: now, snapshot: nil)
        }
        return AgentBuddyRunningTurnEntry(date: now, snapshot: snapshot)
    }
}

struct AgentBuddyRunningTurnView: View {
    let entry: AgentBuddyRunningTurnEntry
    ...
    private var rectangularBody: some View {
        if let snapshot = entry.snapshot {
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 4) {
                    RoundedRectangle(cornerRadius: 3)
                        .fill(AgentBuddyComplicationTint.ginger)
            ...
            .widgetURL(URL(string: "agentbuddy-watch://task/\(snapshot.taskId)"))
        } else {
...
#Preview(as: .accessoryRectangular) {
    AgentBuddyRunningTurnWidget()
} timeline: {
    AgentBuddyRunningTurnEntry.placeholder
    AgentBuddyRunningTurnEntry.empty
}

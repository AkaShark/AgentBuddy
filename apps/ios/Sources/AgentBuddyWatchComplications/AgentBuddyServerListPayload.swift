...
struct AgentBuddyServerListPayload: Codable, Equatable {
...
enum AgentBuddyServerListStore {
    static let appGroup = "group.com.akashark.agentbuddy"
    static let key = "servers.v1"

    static func current() -> AgentBuddyServerListPayload? {
        guard
            let defaults = UserDefaults(suiteName: appGroup),
            let data = defaults.data(forKey: key)
        else { return nil }
        return try? JSONDecoder().decode(AgentBuddyServerListPayload.self, from: data)
    }

    static func write(_ payload: AgentBuddyServerListPayload) {
...
///
/// Shape matches the per-server `AgentBuddyComplicationEntry.Payload` written
/// in the aggregate path, so the same decoder can rehydrate either side.
enum AgentBuddyPerServerComplicationStore {
    static let appGroup = "group.com.akashark.agentbuddy"

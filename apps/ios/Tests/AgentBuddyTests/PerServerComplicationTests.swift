...
/// payload (Task #7). The shared types live in the AgentBuddyWatchComplications
...
final class PerServerComplicationTests: XCTestCase {
    ...
    func testServerListPayloadRoundTripsThroughJSON() throws {
        let payload = AgentBuddyServerListPayload(servers: [
            .init(id: "macbook", displayName: "MacBook Pro"),
            .init(id: "studio", displayName: "Mac Studio"),
        ])

        let data = try JSONEncoder().encode(payload)
        let decoded = try JSONDecoder().decode(AgentBuddyServerListPayload.self, from: data)
    ...
    func testPerServerSnapshotsFilterRunningTaskToOwningServer() throws {
        ...
        let macbook = try JSONDecoder().decode(AgentBuddyComplicationPayload.self, from: macbookData)
        ...
        let studio = try JSONDecoder().decode(AgentBuddyComplicationPayload.self, from: studioData)
    ...
    func testPerServerSnapshotsEmitOfflineWhenWatchUnreachable() throws {
        ...
        let decoded = try JSONDecoder().decode(AgentBuddyComplicationPayload.self, from: data)

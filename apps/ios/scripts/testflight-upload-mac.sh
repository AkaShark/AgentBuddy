...
#   * Archives the `AgentBuddyMac` scheme with destination
#     `generic/platform=macOS,variant=Mac Catalyst` → produces a `.pkg`.
#   * No Live Activity widget to sign (stripped from AgentBuddyMac target).
...
TEAM_ID="${TEAM_ID:-}"
PROVISIONING_PROFILE_SPECIFIER="${PROVISIONING_PROFILE_SPECIFIER:-AgentBuddy Mac App Store}"
...
# `AgentBuddy` distribution profile (same bundle ID, no sandbox entitlement) and
...
if [[ "$TESTFLIGHT_SKIP_BUILD" != "1" ]]; then
    ...
    archived_app="$ARCHIVE_PATH/Products/Applications/AgentBuddy.app"
    ...
    if ! grep -q "com\.apple\.security\.app-sandbox" <<<"$entitlements_xml"; then
        ...
        echo "       points at the Mac App Store profile (not the iOS AgentBuddy distribution profile)." >&2
        exit 1
    fi

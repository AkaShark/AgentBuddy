...
- Task 15: Ruling: CI 先构建 sidecar 再跑测试，且 Rust 测试、契约测试、前端测试只在 arm64 这一路跑 — tauri-build 要求 host triple 的 sidecar 文件存在，计划把 `cargo test` 放在构建 sidecar 之前，两路都会失败；x86_64 这一路没有 host triple 的 sidecar — 代价：Intel 那一路不跑单测，只做打包。
